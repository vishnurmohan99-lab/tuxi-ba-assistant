...
export default function AddPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/add-user-story', {
        ...
        body: JSON.stringify({ featureName, newStoriesRequired: buildStoriesRequired(), additionalNotes }),
      });
  ...
  return (
    <div className="page container">
      <div className="page-header"><h1>Extend Feature</h1><p>Add more user stories to an existing feature while maintaining full consistency.</p></div>
      <div className="card">
        ...
        <div className="form-group">
          <label>New User Stories to Add</label>
        ...
        <div className="form-group">
          ...
          <textarea rows={2} value={additionalNotes} onChange={(e) => setAdditionalNotes(e.target.value)} placeholder="e.g. Keep the same receipt calculation rules." />
        </div>

        {error && <div className="alert alert-error">{error}</div>}

        <button className="btn btn-primary" onClick={handleGenerate}
          disabled={loading || !featureName || filledStories.length === 0}>
          {loading ? <><span className="spinner" /> Generating...</> : `Generate ${filledStories.length > 0 ? filledStories.length : ''} New Stories`}
        </button>
      </div>
