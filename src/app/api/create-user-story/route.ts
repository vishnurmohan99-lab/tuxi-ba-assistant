...
import { getProjectContext } from '@/lib/sheets';

export async function POST(req: NextRequest) {
  try {
    const { featureName, featureDescription, storiesRequired, additionalNotes } = await req.json();
    if (!featureName || !featureDescription || !storiesRequired) {
      return NextResponse.json({ error: 'featureName, featureDescription, and storiesRequired are required.' }, { status: 400 });
    }

    // Auto-load saved project context from Sheets
